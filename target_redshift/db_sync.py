...
class DbSync:
    ...
    def put_to_s3(self, file, stream, count, suffix=""):
        ...
        self.logger.info(
            "Target S3 bucket: {}, local file: {}, S3 key: {}".format(bucket, file, s3_key)
        )
    ...
    def primary_key_merge_condition(self):
        ...
        return " AND ".join(
            [
                "{}.{} = s.{}".format(self.table_name(stream_schema_message["stream"], False), c, c)
                for c in names
            ]
        )
    ...
    def get_table_columns(self, table_schema=None, table_name=None, filter_schemas=None):
        ...
        if table_name is not None:
            sql = sql + " AND LOWER(c.table_name) = '" + table_name.replace('"', "").lower() + "'"
